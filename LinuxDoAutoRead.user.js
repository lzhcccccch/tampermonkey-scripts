...
// 创建控制面板 - 包含所有按钮的垂直排列，带自动隐藏功能
function createControlPanel() {
  // 创建触发区域（用于鼠标悬停检测）
  const triggerArea = document.createElement("div");
  triggerArea.id = "control-trigger";
  triggerArea.style.position = "fixed";
  triggerArea.style.bottom = "50px"; // 修改：放置在底部
  triggerArea.style.left = "0";
  triggerArea.style.width = "10px";
  triggerArea.style.height = "200px";
  triggerArea.style.zIndex = "9998";
  triggerArea.style.backgroundColor = "transparent";
  ...
  controlPanel.style.bottom = "50px"; // 修改：放置在底部
  controlPanel.style.left = "-220px"; // 修改：更靠左隐藏
  controlPanel.style.zIndex = "9999";
  controlPanel.style.display = "flex";
  controlPanel.style.flexDirection = "column";
  controlPanel.style.gap = "8px";
  controlPanel.style.alignItems = "flex-start";
  controlPanel.style.backgroundColor = "rgba(240, 240, 240, 0.95)";
  controlPanel.style.padding = "15px";
  controlPanel.style.borderRadius = "0 10px 10px 0";
  controlPanel.style.boxShadow = "2px 0 15px rgba(0, 0, 0, 0.2)";
  controlPanel.style.transition = "left 0.3s ease-in-out";
  ...
  indicator.style.borderRadius = "0 8px 8px 0";
  ...
  function showPanel() {
    ...
    indicator.innerHTML = "◄";
    indicator.style.backgroundColor = "rgba(255, 107, 107, 0.8)";
  }
  ...
  // 为触发区域添加事件
  triggerArea.addEventListener("mouseenter", showPanel);
  ...
  readButton.style.marginBottom = "5px";
